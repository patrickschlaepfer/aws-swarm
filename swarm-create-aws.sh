...
docker-machine create --driver amazonec2 --amazonec2-access-key $AWS_ACCESS_KEY_ID --amazonec2-secret-key $AWS_SECRET_ACCESS_KEY --amazonec2-zone=a --amazonec2-region "eu-central-1" --amazonec2-vpc-id $AWS_VPC_ID --engine-opt dns=8.8.8.8 aws-mh-keystore
